...
class WorldManager:
    ...
    def spawn_asset(
        ...
        hit = get_physx_scene_query_interface().raycast_closest(
            [x, y, 200], [0, 0, -1], 100000.0
        )

        if hit["hit"]:
            distance = hit["distance"]

            print("trying to spawn at ray cast hit", hit["position"][2], hit)
    ...
    def spawn_loop(
        ...
        for i, n in enumerate(coll):
            ...
            closest_ind = np.ravel_multi_index(
                closest, (len(height_map), len(height_map))
            )
            ...
            for inds in possible_inds:
                ...
                normal_vec += self.calculate_normal(vert1, vert2, c).astype(np.float32)
                ...
                if magnitude > 0:
                    euler_deg = self.axis_angle_to_euler([0, 0, 1], normal_vec)
                    euler_deg[0] = max(euler_deg[0], 5)
                    euler_deg[1] = max(euler_deg[1], 5)
    ...
    def create_terrains(self, terrain_info, mesh_save_path):
        ...
        for key in terrain_info:
            ...
            for i, path in enumerate(files_with_material_id):
                ...
                # prim_p = f'/World/t/class_{mat_name}_{i}'
                # prim_p = f"/World/t/class_{mat_name}/n{i}"
                prim_p = f"/World/t/class_{mat_name}"

                random_rotation = 0.0
                x, y, z = 0, 0, 0
                p = add_reference_to_stage(usd_path=path, prim_path=prim_p)

                self.create_material_and_bind(
                    mat_name, mat_path, prim_p, scale, self.stage
                )
                prim = self.stage.GetPrimAtPath(prim_p)

                for child in prim.GetChildren():
                    if len(child.GetChildren()) > 0:
                        for subchild in child.GetChildren():
                            print("applying collisions for sub child, ", subchild)
                            # collisionAPI = UsdPhysics.ConlisionAPI.Apply(subchild)
                            # sdf
                            utils.setCollider(subchild, approximationShape="none")

                    sem = Semantics.SemanticsAPI.Apply(child, "Semantics")
                    sem.CreateSemanticTypeAttr()
                    sem.CreateSemanticDataAttr()
                    sem.GetSemanticTypeAttr().Set("class")
    ...
    def generate_world_generator(self, obj_path, world_path):
        ...
        return obs_to_spawn, object_dict, height_map, meshGen.normals, meshGen
    ...
    def create_world(self, world_path, obj_path, world):
        ...
        (
            ...
            normals,
            meshGen,
        ) = self.generate_world_generator(world_path, obj_path)
        self._log(f"Terrain mesh generated and imported.")
        self._log(f"Attempting to spawn all objects.")
        print(" world should now play")
        ...
        self._log("Trying to clean up all files")
